...
using DesktopApp;
using MessageSystemCSDesktopApp;

namespace DesktopApp
{
    public partial class ConversationTabPage : TabPage  
    {
        ...
        private void tb_send_message_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                main.Log("Нажаты клавиши Shift + Enter.");
            }
            else if (e.KeyCode == Keys.Enter && !e.Shift)
            {
                main.Log("Нажата клавиша Enter");
                btn_send.PerformClick();                                
                main.Log("Сообщение отправлено.");
            }
        }
        ...
        public void NewMessageFromMe(DateTime messageTimeStamp, string message)
       {
        ...
        private void btn_send_Click(object sender, EventArgs e)
        {
            if (!String.IsNullOrWhiteSpace(tb_send_message.Text) || !String.IsNullOrEmpty(tb_send_message.Text))
            {
                ...
                main.SendMessage(this.UID, Data.KeyManagement.Encrypt(this.PublicKey, tb_send_message.Text.TrimEnd(Environment.NewLine.ToCharArray())));
